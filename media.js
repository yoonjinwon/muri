...
(function () {
  const mediaItems = [
    ...
    {
      url: "assets/media/adv_science.mov",
      siteName: "Advanced Science",
      title: "Featured video",
      video: "assets/media/adv_science.mov",
      favicon: "https://www.google.com/s2/favicons?domain=advancescience.com&sz=64",
    },
  ...
  function renderCard(meta) {
    ...
    const favicon = meta.favicon || faviconUrl(url);

    if (meta.embed === "youtube") return renderYoutubeEmbedCard(meta);

    const media = video
      ? `<div class="media-card-image">
          <video src="${escapeHtml(video)}" preload="metadata" muted autoplay loop playsinline></video>
        </div>`
      : image
    ...
    return `
      <a class="media-card" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">
        ...
        <div class="media-card-body">
          <div class="media-card-kicker">
            <img class="media-favicon" src="${escapeHtml(favicon)}" alt="" aria-hidden="true" />
            <span>${escapeHtml(meta.siteName || domain)}</span>
          </div>
          <div class="media-card-title">${escapeHtml(title)}</div>
          ${desc}
  ...
  const itemByUrl = new Map(normalized.map((item) => [item.url, item]));
  const allLinks = normalized.map((i) => i.url);
  const metaCache = new Map();

  for (const item of normalized) {
    if (item.title || item.description || item.image || item.video || item.siteName || item.favicon) {
      metaCache.set(item.url, { ...item });
    }
  }
  ...
  function updateNavState() {
    const disabled = normalized.length <= PAGE_SIZE;
    if (prevBtn) prevBtn.disabled = disabled;
    if (nextBtn) nextBtn.disabled = disabled;
  }

  function renderCardOrFallback(meta) {
    return meta && (meta.title || meta.description || meta.image || meta.video || meta.embed)
      ? renderCard(meta)
      : renderFallbackCard(meta.url);
  }
  ...
  async function renderPage() {
    ...
    const count = Math.min(PAGE_SIZE, normalized.length);
    const pageItems = [];
    for (let i = 0; i < count; i += 1) {
      pageItems.push(normalized[(start + i) % normalized.length]);
    }

    const initial = pageItems.map((item) => metaCache.get(item.url) || item);
    gridEl.innerHTML = initial.map(renderCardOrFallback).join("");
    ...
    const metas = await Promise.all(
      pageItems.map(async (item) => {
        ...
        const manual = itemByUrl.get(url);
        const existing = cached || manual || item;
        const hasRich =
          existing &&
          (existing.title || existing.description || existing.image || existing.video || existing.embed);

        if (hasRich || !isHttpUrl(url)) return existing;

        try {
          const fetched = await fetchOgMeta(url);
          metaCache.set(url, { ...(manual || item), ...fetched });
    ...
    gridEl.innerHTML = metas.map(renderCardOrFallback).join("");
  }
  ...
  function step(delta) {
    ...
    start = (start + delta) % normalized.length;
  ...
  function flashSelected(btn) {
    ...
    window.setTimeout(() => btn.classList.remove("is-selected"), 180);
  }

  if (prevBtn) {
    prevBtn.addEventListener("click", (e) => {
      e.preventDefault();
      flashSelected(prevBtn);
      step(-PAGE_SIZE);
    });
  }
